...
public sealed class BrandsController : Controller
{
    ...
    public BrandsController(ApplicationDbContext dbContext) => _dbContext = dbContext;

    public async Task<IActionResult> Index(CancellationToken cancellationToken) =>
        View(await _dbContext.Brands.AsNoTracking().OrderBy(item => item.Name).ToListAsync(cancellationToken));

    [HttpGet]
    public async Task<IActionResult> Edit(int? id, CancellationToken cancellationToken)
    {
        if (!id.HasValue) return View(new CatalogEntityViewModel());
        var entity = await _dbContext.Brands.AsNoTracking().SingleOrDefaultAsync(item => item.Id == id, cancellationToken);
    ...
    public async Task<IActionResult> Edit(CatalogEntityViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (await _dbContext.Brands.AnyAsync(item => item.Slug == slug && item.Id != model.Id, cancellationToken))
        {
        ...
        if (model.Id == 0)
        {
            entity = new Brand { CreatedAt = DateTime.UtcNow };
            _dbContext.Brands.Add(entity);
        }
        else
        {
            entity = await _dbContext.Brands.SingleOrDefaultAsync(item => item.Id == model.Id, cancellationToken) ?? throw new InvalidOperationException("Brand not found.");
        }
